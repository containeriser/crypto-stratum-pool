...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SubmitAuxBlock(aux, headerBuffer, coinbaseBuffer, blockHash, callback) {
        ...
        var auxpowVersion = new Buffer([]);
        var personalization = new Buffer([]);
        if(_this.auxes[aux].rpcData.baseversion) {
            //auxpow 2.0

            //base version
            var ver = 0x00000001;
            if(_this.options.coin.reward == "POS") {
                //pos flag
                ver |= 0x00001000;
            }
            if(_this.options.coin.algorithm == "equihash") {
            //if(_this.options.coin.parameters.N == 200 && _this.options.coin.parameters.K == 9) {
                //equihash 200,9 flag
                ver |= 0x00002000;
            }
            if(_this.options.coin.parameters && _this.options.coin.parameters.N == 144 && _this.options.coin.parameters.K == 5) {
                //equihash 144,5 flag
                ver |= 0x00004000;
                //personalization string
                personalization = util.varStringBuffer(_this.options.coin.parameters.personalization);
            }

            auxpowVersion = util.packUInt32LE(ver);
        }
...
/*
        if(_this.auxes.length > 1) {
            console.log("Auxpow Version : "+auxpowVersion.toString("hex"));
            console.log("Personalization: "+personalization.toString("hex"));
            console.log("Coinbase Buffer: "+coinbaseBuffer.toString("hex"));
            console.log("Block Hash     : "+blockHash.toString("hex"));
            console.log("Coinbase Proof : "+coinbaseProof.toString("hex"));
            console.log("Branch Proof   : "+branchProof.toString("hex"));

            console.log("Header         : "+headerBuffer.slice(0,140).toString("hex"));
            console.log("Solution       : "+headerBuffer.slice(140).toString("hex"));
            
            console.log("Aux root hash  : "+_this.auxes[aux].rpcData.hash.toString("hex"));
        }*/
        ...
        _this.auxes[aux].daemon.cmd('getauxblock',
            ...
            function(results) {
                ...
                if(typeof callback == "function") callback(_this.auxes[aux].rpcData.hash, aux);
            });
    }
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            */
            if (isValidBlock) {
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx){
                        ...
                        shareData[isValidBlock ? 'txHash' : 'error'] = tx;
            ...
            // Aux block check / submit
            var auxinfo = [];
            if(!shareData.error) {
                for(var i = 0;i < _this.auxes.length;i++) {
                    var this_auxinfo = {"coin":_this.auxes[i].name,"target":_this.auxes[i].rpcData.target,"isValidBlock":false};

                    if(_this.auxes[i].rpcData.target.ge(shareData.blockBigNum)) {
                        SubmitAuxBlock(i, new Buffer(blockHexInvalid, 'hex').slice(0,80), shareData.coinbaseBuffer, shareData.headerHash, function(hash, aux) {
                            if(!hash) return;
                            CheckBlockAccepted(hash, _this.auxes[aux].daemon, function(accepted, tx, height, value) {
                                ...
                                _this.auxes[aux].daemon.cmd('gettransaction', [tx], function(res) {
                                    ...
                                    if(cmdError) {
                                        ...
                                        _this.emit('auxblock', _this.auxes[aux].name, height, hash, tx, cmdResponse.details[0].amount, shareData.difficulty, shareData.worker);
                                    }
                                });
                                UpdateAuxes();
                        ...
                        this_auxinfo.isValidBlock = true;
                    }
                    
                    auxinfo.push(this_auxinfo);
                }
            }
            
            //always emit share
            _this.emit('share', isValidShare, isValidBlock, shareData, auxinfo);
    ...
    function SetupDaemonInterface(finishedCallback){
        ...
        if (options.auxes) {
          for(var i = 0;i < options.auxes.length;i++) {
              ...
              var a = {};
              a.name = options.auxes[i].coin.name; // I want at least this...
        ...
        _this.auxes[index].daemon.cmd('getauxblock', [],
            function(result){
                if (result.error){
                    ...
                    _this.auxes[index].rpcData.target = bignum.fromBuffer(util.uint256BufferFromHash(_this.auxes[index].rpcData.target || _this.auxes[index].rpcData._target, {endian: 'little', size: 32}));
    ...
    **/
    ...
    this.processBlockNotifyAux = function(blockHash, sourceTrigger) {
        emitLog('Block notification via ' + sourceTrigger);
        UpdateAuxes();
    };
