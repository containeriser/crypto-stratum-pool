...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = !options.coin.isZCashProtocol ? function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask, isSoloMining) {
        ...
        blockHexInvalid = job.serializeBlock(headerBuffer, Buffer.from(soln, 'hex')).toString('hex');
        blockHashInvalid = util.reverseBuffer(headerHash).toString('hex');
        ...
        _this.emit('share', {
            ...
            diff1: evdiff1,
            shareMultiplier: shareMultiplier,
            coinbaseBuffer: headerSolnBuffer,
            txHash: headerSolnBuffer.toString('hex'),
            headerHash: headerHash,
            blockBigNum: headerBigNum,
            blockHash: blockHash,
            blockHashInvalid: blockHashInvalid,
