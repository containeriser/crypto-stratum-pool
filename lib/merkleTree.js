...
MerkleTree.prototype = {
 ...
 // Used to develop steps to prove a single hash is part of a merkle root
    getHashProof: function(h) {
        ...
        //indexOf doesn't work for buffers
        //using a for..in loop sets the iterator to a string value; use pure numbers instead
        var ind = -1;
        for(var i = 0; i < data.length; i++) {
            if(data[i].toString("hex") == h.toString("hex")) {
                ind = i;
                break;
            }
        }
        ...
        var branch_len = 0;
        var hash_buffer = new Buffer([]);
        
        //the side mask is equal to the index of the item in the bottom row of the merkle tree
        var side_mask = ind;
        ...
        for(;data.length > 1;branch_len++) {
            ...
            if(ind % 2 === 0) {
                ...
                // We need left side
                hash_buffer = Buffer.concat([hash_buffer, data[ind-1]]);
                //side_mask += Math.pow(2,branch_len);
            }
            // Calculate the next level of the merkle root.
            var newData = [];
            for(var i = 0;i < data.length;i += 2) newData.push(this.mj(data[i], data[i + 1]));
            data = newData;
